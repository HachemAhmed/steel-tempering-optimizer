...
def plot_interactive_heatmap(graph, output_filename, highlight_points=None, auto_open=False):
    ...
    fig.update_layout(
        ...
        hovermode='closest',
        font=dict(size=12),
        # Position legend horizontally below the plot - FIXED POSITION
        legend=dict(
            orientation='h',
            x=0.5,
            y=-0.15,
            xanchor='center',
            yanchor='top',
            xref='paper',  # Use paper coordinates (fixed)
            yref='paper',  # Use paper coordinates (fixed)

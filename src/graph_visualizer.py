...
def plot_filtered_graph(graph, path, cost, optimize_by, output_image_filename):
    """
    Draws a simplified visual graph.
    It merges the unique logic nodes back into simple labels (e.g. 'Time: 10s') 
    to avoid visual clutter ('spaghetti effect').
    """
    ...
    # 1. Create Simplified Display Graph
...
def plot_hardness_heatmap(graph, output_image_filename, highlight_point=None, winner_hardness=None):
    """
    Generates a Scatter Plot Heatmap.
    Solves overplotting by Deterministic Sorting (hardest on top).
    Highlights the winner with a clean empty circle.
    """
    if graph is None or graph.number_of_nodes() == 0:
        print("Empty graph, skipping heatmap.")
        return

    # Collect real data points from unique nodes
    data_points = []
    for node, data in graph.nodes(data=True):
        if data.get('type') == 'temp':
            ...
            data_points.append((temp_val, time_val, hardness_val))

    if not data_points:
        print("Not enough data for heatmap.")
        return

    print(f"Generating Heatmap with {len(data_points)} points...")

    # --- DETERMINISTIC SORTING ---
    # Sort by hardness. This ensures the visual stacking order is consistent.
    # Higher hardness points will be drawn on top of lower hardness points (or vice-versa).
    data_points.sort(key=lambda x: x[2])

    temperatures = [p[0] for p in data_points]
    times = [p[1] for p in data_points]
    hardnesses = [p[2] for p in data_points]
    ...
    # 1. Plot Valid Options (Background)
    # alpha=1.0 (solid) ensures colors match the colorbar perfectly
    ...
    # 2. Highlight Optimal Solution
    if highlight_point:
        best_temp, best_time = highlight_point
        
        # A) Redraw winner dot on top (zorder=5)
        ...
        # B) Draw Empty Black Frame (zorder=10)
